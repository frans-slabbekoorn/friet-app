...
export const fallbackLanguage: { [key: string]: string } = translations.nl;
...
export const getLanguage = async (): Promise<any> => {
    ...
    const languageIndex: number = languages.findIndex(
        (language): boolean => language === selectedLanguage,
    );
